...
試し打ちの目印は `is_test` 列である。この列に `true` が立つのは次のどちらかのとき。

1. **参加者IDの頭が `curltest-` / `uitest-`**（クラウドソーシングの作業者IDがこの形に
   なることはないので、本物の参加者と取り違える心配がない）
2. ページが **`is_test=1` を送ってきた**とき。`experiment/transfer_config.js` の
   掲載前フラグ `pre_launch` が `true` のあいだ、ページは参加者IDに関係なくこれを送る
   （2026-08-22 に追加。掲載前チェックリスト0節）

印の付いた行だけが `transfer_purge_test` で消せるので、本物の参加者のデータには手が届かない。
**印の付いた人は名簿の人数の勘定にも入らない**（本番の連番を消費しない）ので、
動作確認をしても聴覚と視覚の交互の振り分けはずれない。
