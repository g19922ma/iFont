...
function finishHTML(durS) {
  if (window.PROD && PROD.enabled) return PROD.completionHTML(durS, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p><b>このページを開いたタスクの画面に戻り</b>、タスクに書かれたヒントの文字に対応する<b>完了コード</b>を、下の表から選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
  ...
  try {
    ...
    if (window.PROD && PROD.enabled) {
      ...
      if (resumeState && resumeState.completed) {
        screenEl.innerHTML = PROD.completionHTML(resumeState.duration_s || 0, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p><b>このページを開いたタスクの画面に戻り</b>、タスクに書かれたヒントの文字に対応する<b>完了コード</b>を、下の表から選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
        return;
      }
    }
    await startSession();
