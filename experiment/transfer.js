...
//   ・completion_code … 参加者が募集サイトの回答欄に貼る6文字。これで照合する
...
async function showResults() {
  ...
  // ⚠ **最後の1問の送信が終わるのを少し待つ。**
  //   1問ごとの送信は待たずに投げている（参加者を待たせないため）ので、
  //   最後の問題の送信は**まだ空を飛んでいる**ことがある。
  //   すぐ下の送り直しを先に走らせると、その1問を拾い損ねる。
  await sleep(1500);
...
function finishHTML(durS) {
  if (window.PROD && PROD.enabled) return PROD.completionHTML(durS, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p>下の<b>完了コード</b>を、<b>このページを開いたタスクの画面に戻り</b>、「完了コード1文字目〜3文字目」の選択欄で順番に選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
...
// 通信についての注意。**2026-08-25 に栗原先生のご指摘で足した。**
//   先生が電車内で通されたとき、17分間ずっと送信が失敗し、
//   70問のうち60問ぶんが記録に届かなかった。
//   ⚠ **貯めて送り直す仕組みを入れたので取り戻せる**が、
//     移動中はそもそも避けてもらうほうが確実である。
//   聴覚・視覚の両方の説明に出す。
const NET_NOTE = "<li><b>通信が安定した場所で行ってください。</b>"
               + "移動中など電波が不安定だと、回答が記録に残らないことがあります。</li>";
...
(async function () {
  try {
    ...
    if (window.PROD && PROD.enabled) {
      ...
      // ⚠ **前回に送れなかった記録を読み戻す。**（loadState のあと＝参加者IDが
      //   保存時のものへ戻ったあとに呼ぶこと。控えは参加者IDごとに分けてある。）
      //   ここを呼ばないと、ブラウザを閉じた時点で貯めたぶんが取り出せなくなる。
      loadPending();
      if (pendingRecords.length) {
        console.warn(`[transfer] 前回送れなかった記録が ${pendingRecords.length} 件あります。`
                   + `課題を進めながら送り直します。`);
        flushPending(5);
      }
      ...
      if (resumeState && resumeState.completed) {
        screenEl.innerHTML = PROD.completionHTML(resumeState.duration_s || 0, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p>下の<b>完了コード</b>を、<b>このページを開いたタスクの画面に戻り</b>、「完了コード1文字目〜3文字目」の選択欄で順番に選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
        return;
      }
    }
    await startSession();
