...
function finishHTML(durS) {
  if (window.PROD && PROD.enabled) return PROD.completionHTML(durS, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p><b>このページを開いたタスクの画面に戻り</b>、チェック設問のヒントに対応する番号を、下の表を見ながら選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
  ...
  try {
    ...
    if (window.PROD && PROD.enabled) {
      ...
      if (resumeState && resumeState.completed) {
        screenEl.innerHTML = PROD.completionHTML(resumeState.duration_s || 0, { hideMeta: true, hintTable: true, hideCode: true, codeNote: `<p><b>このページを開いたタスクの画面に戻り</b>、チェック設問のヒントに対応する番号を、下の表を見ながら選んで提出してください。</p><p class="muted" style="font-size:13px">提出するまで、報酬のお支払い手続きが始まりません。</p>` });
        return;
      }
    }
    await startSession();
