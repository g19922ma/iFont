...
(function (root) {
  ...
  //
  // ⚠ **2026-08-26 に、一覧が無いフェーズの既定を「試し打ち」に変えた。**
  //
  //   それまでは「一覧が設定に無いフェーズは前置きだけで判定する」＝
  //   **一覧を登録する前に動作確認すると、その記録が本番として残る**作りだった。
  //   検証(test)・群C(comfort)・向きの実験(wipedir)はこれから回すので、
  //   準備中に必ず自分で開いて確かめる。そのたびに本番データが濁るのは避けたい。
  //
  //   本番の参加者は募集サイトの設問ごとに違うURLから来るので、
  //   **必ず配布した一覧のどれかのID**になる。だから
  //   **一覧が無い＝まだ誰にも配っていない＝本番ではありえない**と見なしてよい。
  //
  //   ⚠ 掲載する前に **write_distributed_ids.py で一覧を登録すること。**
  //     登録し忘れると、本物の参加者の記録まで is_test=TRUE になる
  //     （消えはしない。フラグを直して数え直せば復旧できる）。
  //     一覧を使わずに1本のURLで募集する場合は、そのフェーズ名を
  //     設定の open_phases に足すと、前置きだけの判定（従来の挙動）に戻る。
  function isTestRun(pid, phase) {
    if (preLaunch() || isTestPid(pid)) return true;
    var ids = distributedIds(phase);
    if (ids && ids.length) return ids.indexOf(String(pid || "")) < 0;
    var open = (cfg().open_phases || []);
    return open.indexOf(String(phase || "")) < 0;   // 一覧が無ければ既定で試し打ち
  }
