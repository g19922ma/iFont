...
const VERSION = "3.49";   // パイロットのバージョン(細かい改変ごとにインクリメント)
...
const POOL_NAMES = { legacy_metan: "参考: 旧 四国めたん", cand94: "候補: 中部つるぎ",
  ...
  // 1モーラ0.2秒の中で、子音と母音に時間をどう配るかの違い(試作)。本番は子音長を自然値のまま
  // 残して母音で0.2秒に帳尻を合わせるので、子音の長い音は母音が痩せる(「し」の母音は42.7ms)。
  // uniform200 は子音と母音を同じ倍率で伸縮するので、自然な子音と母音の比が保たれる。
  // モーラ長・話者・音高・品質処方は本番と同じ。
  uniform200: "候補: 一様伸縮200ms(方式2)" };
...
async function preload() {
  ...
  await Promise.all(have.map(async ch => {
    ...
    if (bar) bar.style.width = `${Math.round(done / have.length * 100)}%`;
  }));
}
