...
def smooth3(y):
    ...
    測った水準が7点しかないので、これ以上強く均すと段差の位置が動いてしまう。
...
def curves_from_trials(rows, cfg, gamma, lapse_arg, full_len):
    ...
    計画書 7.1 の「生成用」に対応する。除外の規則もここで当てる。
      ...
      ・下見だけ足した時点/水準は入れない
        (audio.pilot_extra_gates.enabled / visual.pilot_extra_levels.enabled が true のときのみ除外。
         false のときは、同じ値が gates_ms 本体に載っている正規の時点なので落とさない)
