...
<b>曲線の立ち上がりが20ミリ秒より手前にある疑いがあります。</b>
そこで下見だけ、より早い <b>10ミリ秒</b>を1時点足しました（<code>audio.pilot_extra_gates</code>）。
ただし10ミリ秒の刺激は、終端の5ミリ秒フェードのぶん、そのままの大きさで鳴るのが頭の約5ミリ秒だけになります。
<b>この時点は床の目印として読み、生成の材料には使いません。</b>本番の掲載前に切ります。</p>
</div>
</div>
...
なお下見用に水準を足したことで、聴覚は 94問 → <b>108問</b>（約11.5分見込み）、
視覚は 117問 → <b>134問</b>（約8.5分見込み）に増えています。
<b>聴覚は掲載の「約10分」を超える見込みなので、掲載直前にタイトルの分数を判断してください。</b></p>
...
<li><b>下見だけの仕掛けを本番前に外す</b>：<code>visual.pilot_extra_levels.enabled</code> と <code>audio.pilot_extra_gates.enabled</code> を false へ。時点を変えたら打ち切りWAVを作り直します。<code>calib_speed_probe</code> のほうは較正フェーズのあいだ入れたままにします。</li>
</ul>
